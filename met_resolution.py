...
def constructModel(Hist, bkg_hist,  m, um,uM, BKGSubtraction, eemm, cut, var, plot_name):
    print 'here now'
    plot = Canvas.Canvas("met/resolution_plots/"+var+"_vs_"+str(reg.dependence)+"_"+eemm+"_"+plot_name+"_"+cut , "png,pdf",0.6, 0.7, 0.8, 0.9)
    f = 0
    efwhm = 0
    v_m.setVal(m)
    v_m.setRange(um,uM) 
...
#    regions.append(region)                 
    ...
    for reg in regions:
        if reg.name == 'Raw 1 ME_{T}':
            regname = 'met_raw'       

        if reg.name == 'Type 1 ME_{T}':
            regname = 'met_type1'       

        if reg.rvars == 'met_uPara_zll+zll_pt':
            label = 'u_{|| }'
            var = 'uPara'

        if reg.rvars == 'met_uPerp_zll':    
            var = 'uPerp'
            label = 'u_{#perp}'
        graph_ee = TGraphErrors()
        graph_mm = TGraphErrors()
        ...
        for i in cutsList:
            ...
            for tree in trees:
                ...
                if tree.name == 'dy_ee':                                                                                                                                     
                    ...
                    dy_hist_mm = tree.getTH1F(lumi, reg.name+i, reg.rvars,40, -150., 150.,  i, '', reg.name)    
