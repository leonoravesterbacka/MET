...
#also this one compares 74X raw met to 76X raw met for now, 
# uPerp and uPara is done separately
import ROOT as r
...
if __name__ == "__main__":
    ...
    treeDY_ee_old = Sample.Tree(helper.selectSamples(opts.sampleFile, dy_ee_old, 'dy_ee'), 'dy_ee', 0)
    treeDY_mm_old = Sample.Tree(helper.selectSamples(opts.sampleFile, dy_mm_old, 'dy_mm'), 'dy_mm', 0)
  ...
  #          [range(-100,300,50)],
  #          False)
  #  regions.append(region)             ##############this one can only do uPerp and uPara one at the time     
...
################## nvtx
    ...
    trees = [treeDY_ee, treeDY_ee_old, treeDY_mm, treeDY_mm_old]
    isOld = [False, True, False, True]
    for reg in regions:
       if reg.name == 'Raw ME_{T}':
           print 'met raw set here'
           regname = 'met_raw'       
       ...
       for i in cutsList:
           ...
           for tree in trees:
               ...
               if tree.name == 'dy_ee':
                   if isOld[trees.index(tree)] == True:
                       print 'doing old'
                       dy_ee_raw_old = tree.getTH1F(lumi, regname+i, reg.rvars,40, -100., 100.,  i, '', regname)
                       dy_ee_raw_old_Hist = RooDataHist("dy_raw_old","dy_raw_old"+i, RooArgList(x), dy_ee_raw_old)
                       dy_ee_Bkg_raw_old = RooDataHist()                                                          
                       m_dy_ee_raw_old = dy_ee_raw_old.GetMean()                            
                       um_dy_ee_raw_old = dy_ee_raw_old.GetMean()-dy_ee_raw_old.GetRMS()
                       uM_dy_ee_raw_old = dy_ee_raw_old.GetMean()+dy_ee_raw_old.GetRMS()
                   else:
                       print 'doing new'
                       dy_ee_raw = tree.getTH1F(lumi, regname+i, reg.rvars,40, -100., 100.,  i, '', regname)
                       print 'dy_ee_raw', dy_ee_raw
                       dy_ee_raw_Hist = RooDataHist("dy_ee_raw","dy_ee_raw"+i, RooArgList(x), dy_ee_raw)
                       dy_ee_Bkg_raw = RooDataHist()                                                          
                       m_dy_ee_raw = dy_ee_raw.GetMean()                            
                       um_dy_ee_raw = dy_ee_raw.GetMean()-dy_ee_raw.GetRMS()
                       uM_dy_ee_raw = dy_ee_raw.GetMean()+dy_ee_raw.GetRMS()                                         
               if tree.name == 'dy_mm':
                   if isOld[trees.index(tree)] == True:
                       print 'doing old'
                       dy_mm_raw_old = tree.getTH1F(lumi, regname+i, reg.rvars,40, -100., 100.,  i, '', regname)
                       dy_mm_raw_old_Hist = RooDataHist("dy_raw_old","dy_raw_old"+i, RooArgList(x), dy_mm_raw_old)
                       dy_mm_Bkg_raw_old = RooDataHist()                                                          
                       m_dy_mm_raw_old = dy_mm_raw_old.GetMean()                            
                       um_dy_mm_raw_old = dy_mm_raw_old.GetMean()-dy_mm_raw_old.GetRMS()
                       uM_dy_mm_raw_old = dy_mm_raw_old.GetMean()+dy_mm_raw_old.GetRMS()
                   else:
                       print 'doing new'
                       dy_mm_raw = tree.getTH1F(lumi, regname+i, reg.rvars,40, -100., 100.,  i, '', regname)
                       dy_mm_raw_Hist = RooDataHist("dy_raw_mm","dy_raw_mm"+i, RooArgList(x), dy_mm_raw)
                       dy_mm_Bkg_raw = RooDataHist()                                                          
                       m_dy_mm_raw = dy_mm_raw.GetMean()                            
                       um_dy_mm_raw = dy_mm_raw.GetMean()-dy_mm_raw.GetRMS()
                       uM_dy_mm_raw = dy_mm_raw.GetMean()+dy_mm_raw.GetRMS()                                         
          ...
          # data_error_mm.append(data_e_mm)
           #mc_f_type, mc_e_type = constructModel(dy_Hist_type, dy_BkgHist_type, m_dy_type, um_dy_type, uM_dy_type, False, 'type', str(cutsList.index(i)), var, 'MC')
           mc_f_raw_ee, mc_e_raw_ee = constructModel(dy_ee_raw_Hist, dy_ee_Bkg_raw, m_dy_ee_raw, um_dy_ee_raw, uM_dy_ee_raw, False, 'ee_raw', str(cutsList.index(i)), var, 'MC')
           mc_f_raw_mm, mc_e_raw_mm = constructModel(dy_mm_raw_Hist, dy_mm_Bkg_raw, m_dy_mm_raw, um_dy_mm_raw, uM_dy_mm_raw, False, 'mm_raw', str(cutsList.index(i)), var, 'MC')
           mc_f_raw_old_ee, mc_e_raw_old_ee = constructModel(dy_ee_raw_old_Hist, dy_ee_Bkg_raw_old, m_dy_ee_raw_old, um_dy_ee_raw_old, uM_dy_ee_raw_old, False, 'ee_raw_old', str(cutsList.index(i)), var, 'MC')
           mc_f_raw_old_mm, mc_e_raw_old_mm = constructModel(dy_mm_raw_old_Hist, dy_mm_Bkg_raw_old, m_dy_mm_raw_old, um_dy_mm_raw_old, uM_dy_mm_raw_old, False, 'mm_raw_old', str(cutsList.index(i)), var, 'MC')
          # mc_fwhms_type.append(mc_f_type)
          # mc_error_type.append(mc_e_type)

           mc_fwhms_raw_ee.append(mc_f_raw_ee)
           mc_fwhms_raw_old_ee.append(mc_f_raw_old_ee)
           mc_error_raw_ee.append(mc_e_raw_ee)
           mc_error_raw_old_ee.append(mc_e_raw_old_ee)
           mc_fwhms_raw_mm.append(mc_f_raw_mm)
           mc_fwhms_raw_old_mm.append(mc_f_raw_old_mm)
           mc_error_raw_mm.append(mc_e_raw_mm)
           mc_error_raw_old_mm.append(mc_e_raw_old_mm)
        
      
      
      # ratio_ee.append(data_f_ee/mc_f_ee)
      # ratio_mm.append(data_f_mm/mc_f_mm)
       print 'mc_fwhms_raw_ee', mc_fwhms_raw_ee
       print 'mc_fwhms_raw_old_ee', mc_fwhms_raw_old_ee
       print 'mc_error_fwhms_raw_ee', mc_error_raw_ee
       print 'mc_error_fwhms_raw_old_ee', mc_error_raw_old_ee
       print 'mc_fwhms_raw_mm', mc_fwhms_raw_mm
       print 'mc_fwhms_raw_old_mm', mc_fwhms_raw_old_mm
       print 'mc_error_fwhms_raw_mm', mc_error_raw_mm
       print 'mc_error_fwhms_raw_old_mm', mc_error_raw_old_mm
       
       print binposition
       print binerror
      # plot_da = Canvas.Canvas("met/resolution_plots/"+var+"_vs_"+str(reg.dependence)+"_fit", "png,pdf",0.6, 0.65, 0.85, 0.82)        
      ...
      # mg.GetYaxis().SetTitle("#sigma("+reg.rvars+")")
      ...
      # del  graph_ee, graph_mm,  mg, leg
       plot_mc_ee = Canvas.Canvas("met/resolution_plots/"+regname+"_"+var+"_vs_"+str(reg.dependence)+"_76vs74_ee", "png",0.5, 0.5, 0.8, 0.8)       
       #graph_type = TGraphErrors(len(binposition), array("f", binposition), array("f", mc_fwhms_type), array("f", binerror), array("f", mc_error_type))
       graph_raw = TGraphErrors(len(binposition), array("f", binposition), array("f", mc_fwhms_raw_ee), array("f", binerror), array("f", mc_error_raw_ee))
       graph_raw_old = TGraphErrors(len(binposition), array("f", binposition), array("f", mc_fwhms_raw_old_ee), array("f", binerror), array("f", mc_error_raw_old_ee))
       graph_raw.SetMarkerColor(46)
       graph_raw_old.SetMarkerColor(9)
       graph_raw.SetMarkerStyle(23)
       graph_raw_old.SetMarkerStyle(22)
       graph_raw.SetTitle("Raw")
       graph_raw_old.SetTitle("Raw 74X")
       mg = TMultiGraph() 
       mg.Add(graph_raw)
       mg.Add(graph_raw_old)
       mg.Draw("AP")
       ...
       leg.SetBorderSize(0)
       leg.Draw()
       leg.AddEntry(graph_raw_old, reg.name+" 74X", "P")
       leg.AddEntry(graph_raw, reg.name+" 76X", "P")
       plot_mc_ee.save(0, 0, 0, lumi)                                                                                                                                        
       del leg, graph_raw, graph_raw_old,  mc_fwhms_raw_ee, mc_fwhms_raw_old_ee
       plot_mc_mm = Canvas.Canvas("met/resolution_plots/"+regname+"_"+var+"_vs_"+str(reg.dependence)+"_76vs74_mm", "png",0.5, 0.5, 0.8, 0.8)       
       graph_raw = TGraphErrors(len(binposition), array("f", binposition), array("f", mc_fwhms_raw_mm), array("f", binerror), array("f", mc_error_raw_mm))
       graph_raw_old = TGraphErrors(len(binposition), array("f", binposition), array("f", mc_fwhms_raw_old_mm), array("f", binerror), array("f", mc_error_raw_old_mm))
       graph_raw.SetMarkerColor(46)
       graph_raw_old.SetMarkerColor(9)
       graph_raw.SetMarkerStyle(23)
       graph_raw_old.SetMarkerStyle(22)
       ...
       leg.AddEntry(graph_raw_old, reg.name+" 74X", "P")
       leg.AddEntry(graph_raw, reg.name+" 76X", "P")
       plot_mc_mm.save(0, 0, 0, lumi)                                                                                                                                        
       del leg, graph_raw, graph_raw_old, mc_fwhms_raw_mm, mc_fwhms_raw_old_mm
