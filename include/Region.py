...
class region():
    def __init__(self, name, cuts, rvars, dependence, bins,  doData):
        self.name      = name
        self.cuts      = cuts
        self.rvars     = rvars
    ...
    def setVariables(self):
        for v in self.rvars:
            if v == 'met':
                ...
                self.met_dy_mm   = collection(self.bins[self.rvars.index(v)], v)
                self.met_dy_ee = collection(self.bins[self.rvars.index(v)], v)
            #    self.met_uPara_zll/zll_pt_dy = collection(self.bins[self.rvars.index(v)], v)
            
            if v == 'metPuppi' :
                self.metPuppi_dy     = collection(self.bins[self.rvars.index(v)], v)
                self.metPuppi_dy_mm     = collection(self.bins[self.rvars.index(v)], v)
                self.metPuppi_dy_ee    = collection(self.bins[self.rvars.index(v)], v)
            #    self.metPuppi_uPara_zll/zll_pt_dy = collection(self.bins[self.rvars.index(v)], v)
